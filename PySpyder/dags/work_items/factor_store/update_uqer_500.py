...
def create_factor_analysis(ds, **kwargs):
    ...
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_uqer', factor_name, 'risk_neutral')
    ...
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_uqer', factor_name, 'top_100')

    # small universe, risk_neutral
    return_table = factor_uqer_analysis(factor_name, ref_date, use_only_index_components=False, risk_neutral=True)
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_big_universe_uqer', factor_name, 'risk_neutral')

    # small universe, top_100
    return_table = factor_uqer_analysis(factor_name, ref_date, use_only_index_components=False, risk_neutral=False)
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_big_universe_uqer', factor_name, 'top_100')


factor_table = pd.read_sql('Describe factor_uqer', engine)

start_date = dt.datetime(2017, 1, 3)
dag_name = 'update_uqer_500_analysis'
