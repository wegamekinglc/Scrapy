...
def factor_uqer_one_day(factor_name, ref_date, use_only_index_components=False, risk_neutral=True):
    ...
    sql = "select factor_uqer.Code, factor_uqer.{factor_name}, factor_data.申万一级行业, {risk_factors}," \
          " return_500.D1LogReturn, return_500.isTradable from factor_uqer".format(factor_name=factor_name,
                                                                                  risk_factors=risk_factors)
    sql += " join factor_data on factor_uqer.Date = factor_data.Date and factor_uqer.Code = factor_data.Code" \
           " join risk_factor_500 on factor_uqer.Date = risk_factor_500.Date and factor_uqer.Code = risk_factor_500.Code" \
           " join return_500 on factor_uqer.Date = return_500.Date and factor_uqer.Code = return_500.Code" \
...
def factor_uqer_analysis(factor_name, ref_date, use_only_index_components=False, risk_neutral=True):

    previous_day = advanceDateByCalendar('china.sse', ref_date, '-1b').strftime('%Y-%m-%d')

    weights, analysis = factor_uqer_one_day(factor_name, ref_date, use_only_index_components, risk_neutral)

    if weights is None:
        logger.warning("No data for '{0}' on {1}".format(factor_name, ref_date))
        return

    previous_weight, _ = factor_uqer_one_day(factor_name, previous_day, use_only_index_components, risk_neutral)
...
def create_factor_analysis(ds, **kwargs):
    ...
    return_table = factor_uqer_analysis(factor_name, ref_date, use_only_index_components=True, risk_neutral=True)
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_uqer_300', factor_name, 'risk_neutral')

    # small universe, top_100
    return_table = factor_uqer_analysis(factor_name, ref_date, use_only_index_components=True, risk_neutral=False)
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_uqer_300', factor_name, 'top_100')

    # small universe, risk_neutral
    return_table = factor_uqer_analysis(factor_name, ref_date, use_only_index_components=False, risk_neutral=True)
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_big_universe_uqer_300', factor_name, 'risk_neutral')

    # small universe, top_100
    return_table = factor_uqer_analysis(factor_name, ref_date, use_only_index_components=False, risk_neutral=False)
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_big_universe_uqer_300', factor_name, 'top_100')


factor_table = pd.read_sql('Describe factor_uqer', engine)

start_date = dt.datetime(2017, 1, 1)
dag_name = 'update_uqer_300_analysis'
...
if __name__ == '__main__':
    create_factor_analysis(None, next_execution_date='2017-01-05', factor_name='BearPower')