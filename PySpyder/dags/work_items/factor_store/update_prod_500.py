...
def prod_500_one_day(factor_name, ref_date, use_only_index_components=False, risk_neutral=True):
    ...
    sql += " join factor_data on prod_500.Date = factor_data.Date and prod_500.Code = factor_data.Code" \
           ...
           " join return_500 on prod_500.Date = return_500.Date and prod_500.Code = return_500.Code" \
           " where prod_500.Date = '{ref_date}'".format(ref_date=ref_date)
...
def prod_500_analysis(factor_name, ref_date, use_only_index_components=False, risk_neutral=True):

    previous_day = advanceDateByCalendar('china.sse', ref_date, '-1b').strftime('%Y-%m-%d')

    weights, analysis = prod_500_one_day(factor_name, ref_date, use_only_index_components, risk_neutral)

    if weights is None:
        logger.warning("No data for '{0}' on {1}".format(factor_name, ref_date))
        return

    previous_weight, _ = prod_500_one_day(factor_name, previous_day, use_only_index_components, risk_neutral)
...
def create_factor_analysis(ds, **kwargs):
    ...
    return_table = prod_500_analysis(factor_name, ref_date, use_only_index_components=True, risk_neutral=True)
    ...
    return_table = prod_500_analysis(factor_name, ref_date, use_only_index_components=True, risk_neutral=False)
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance', factor_name, 'top_100')

    # small universe, risk_neutral
    return_table = prod_500_analysis(factor_name, ref_date, use_only_index_components=False, risk_neutral=True)
    if return_table is not None:
        upload(ref_date, return_table, destination, 'performance_big_universe', factor_name, 'risk_neutral')

    # small universe, top_100
    return_table = prod_500_analysis(factor_name, ref_date, use_only_index_components=False, risk_neutral=False)
    if return_table is not None:
